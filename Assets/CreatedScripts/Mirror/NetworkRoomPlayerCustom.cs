...
namespace Mirror
{
    ...
    [AddComponentMenu("Network/Network Room Player Custom")]
    [HelpURL("https://mirror-networking.gitbook.io/docs/components/network-room-player")]
    public class NetworkRoomPlayerCustom : NetworkBehaviour
    {
        /// <summary>
        ...
        public int netconnid = -1;

        public Action OnSteamIDChanged;
        ...
        {
            if (NetworkManager.singleton is NetworkRoomManagerCustom room)
            {
                ...
                if (isOwned)
                {
                    ...
                    if(NetworkClient.active)
                    {
                        NetworkClient.Send(new steamIdMess() { id = SteamClient.SteamId.Value });
                    }
                }

                room.GetEmplyLobbyPlayerStats()?.Setup(this);
            }
        ...
        {
                steamId = sidm.id.ToString();
        }

        public virtual void OnDisable()
        {
            if (NetworkClient.active && NetworkManager.singleton is NetworkRoomManagerCustom room)
            {
        ...
        public virtual void ReadyStateChanged(bool oldReadyState, bool newReadyState) {
            ((NetworkRoomManagerCustom)NetworkRoomManagerCustom.singleton).CheckReadyToBegin();
        }

        public virtual void SteamIdChanged(string oldId, string newId)
        {
                steamName = FizzyFacepunchLobby.GetSteamName(newId);
            OnSteamIDChanged?.Invoke();
        }
        ...
        public virtual void OnGUI()
        {
            ...
            NetworkRoomManagerCustom room = NetworkManager.singleton as NetworkRoomManagerCustom;
            if (room)
            {
                if (!room.showRoomGUI)
                    return;
