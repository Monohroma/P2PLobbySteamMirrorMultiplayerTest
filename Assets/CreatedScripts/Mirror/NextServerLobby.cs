...
{
    public class NextServerLobby : NextCommon, IServer
    {
        ...
        private NextServerLobby(int maxConnections)
        {
        ...
        public string GetSteamName(int mirrorId)
        {
            SteamId s;
            if (steamIDToMirrorID.TryGetValue(mirrorId, out s))
            {
                if(s.IsValid)
                    return new Friend(s).Name;
            }
            return new Friend(SteamClient.SteamId).Name;
        }
        ...
        public static NextServerLobby CreateServer(FizzyFacepunchLobby transport, int maxConnections)
        {
            NextServerLobby s = new NextServerLobby(maxConnections);
        ...
        private void Host()
        {
            ...
            SteamMatchmaking.OnLobbyMemberJoined += OnMemberEntered;
        ...
        {
            if (r != Result.OK) {
                Debug.LogError("Can't create steam lobby, shutdowning...");
                Shutdown();
                return;
            }
            currentLobby = l;
            Debug.Log(l.Id);
            Debug.Log(l.Owner.Name);
            l.SetFriendsOnly();
            l.SetJoinable(true);
            l.SetData("HostAddress", SteamClient.SteamId.AccountId.ToString());
        }

        private void OnMemberEntered(Lobby l, Friend f)
            {
            Debug.Log("Entered: "+f.Name);
            }
