...
{
  public class NextClientLobby : NextCommon, IClient
  {
    ...
    private NextClientLobby(FizzyFacepunchLobby transport)
    {
      ConnectionTimeout = TimeSpan.FromSeconds(Math.Max(1, transport.Timeout));
      BufferedData = new List<Action>();
    }

    public static NextClientLobby CreateClient(FizzyFacepunchLobby transport, string host)
    {
      NextClientLobby c = new NextClientLobby(transport);
    ...
    public static NextClientLobby CreateClient(FizzyFacepunchLobby transport, Lobby l)
    {
        NextClientLobby c = new NextClientLobby(transport);
    ...
    private async void Connect(string host)
    {
      ...
      finally
      {
        ...
        private async void Connect(SteamId steamId)
        {
            cancelToken = new CancellationTokenSource();
            SteamNetworkingSockets.OnConnectionStatusChanged += OnConnectionStatusChanged;
            ...
            try
            {
                ...
                if (await Task.WhenAny(connectedCompleteTask, timeOutTask) != connectedCompleteTask)
                {
                    ...
                    {
                        Debug.LogError($"Connection to {steamId.AccountId} timed out.");
                    }
